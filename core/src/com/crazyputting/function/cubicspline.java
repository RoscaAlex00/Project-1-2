...
public class cubicspline extends NewtonInterpolator implements DhbInterfaces.OneVariableFunction{
   ...
   private void computeSecondDerivatives()
   {
     ...
     for(int i=1;i<n-1;i++) {
         ...
         s = (points.xValueAt(i) - points.xValue(i - 1)) * invStep2;
     ...
     if(Double.isNaN(endPointDerivative))
         ...
         s = 3.0 / (points.xValueAt(n - 1) - points.xValueAt(n - 2))
                 * (endPointDerivative - (points.yValueAt(n - 1) - points.yValueAt(n - 2))
                 / (points.xValueAt(n - 1) - points.xValueAt(n - 2)));
     }
   ...
   public double value(double x) {
       ...
       return a * points.yValueAt(n1) + b * points.yValueAt(n2)
               + (a * (a * a - 1) * coefficients[n1] + b * (b * b - 1) * coefficients[n2])
               * step * step / 6;
   }
